...
Class Master extends DBConnection {
	...
	function save_transaction(){
		...
		$data = "";
		foreach($_POST as $k =>$v){
			if(in_array($k,array('code','client_name','client_contact','client_purpose','client_or','client_station','client_meter','client_address','total_amount','paid_amount','balance','payment_status','status'))){
				if(!is_numeric($v))
					$v = $this->conn->real_escape_string($v);
				if(!empty($data)) $data .=",";
				$data .= " `{$k}`='{$v}' ";
